...
df['stoch_k'] = ta.stochrsi(close=df['Adj Close'],length=20).iloc[:,0]
df['stoch_d'] = ta.stochrsi(close=df['Adj Close'],length=20).iloc[:,1]
df['bb_lower'] = ta.bbands(close=df['Adj Close'],length=20).iloc[:,0]
df['bb_upper'] = ta.bbands(close=df['Adj Close'],length=20).iloc[:,2]
...
class Strategy(Strategy):
    ...
    def init(self):
        ...
        sma1 = ta.sma(close,self.n1)
        sma2 = ta.sma(close,self.n2)
